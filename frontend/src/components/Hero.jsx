              ...
              <div className="relative z-10 space-y-6">
                ...
                <form onSubmit={handleApply} className="space-y-4">
                  ...
                  {userType === 'employer' ? (
                    ...
                    <>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <input type="text" required value={heroForm.qualification} onChange={e => setHeroForm({...heroForm, qualification: e.target.value})} placeholder="Highest Qualification (e.g. B.Tech, MBA)" className="w-full bg-white/5 border border-white/10 px-6 py-3.5 text-white rounded-2xl focus:border-orange-500 outline-none text-[11px] font-medium placeholder:text-white/20" />
                        <input type="text" required value={heroForm.experience} onChange={e => setHeroForm({...heroForm, experience: e.target.value})} placeholder="Experience (e.g. 2 Years, Fresher)" className="w-full bg-white/5 border border-white/10 px-6 py-3.5 text-white rounded-2xl focus:border-orange-500 outline-none text-[11px] font-medium placeholder:text-white/20" />
                      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
          ...
          <div className="relative w-full max-w-lg bg-[#020617] border border-white/10 rounded-3xl shadow-2xl overflow-hidden animate-fade-in-up">
            ...
            <div className="p-6 sm:p-8">
              ...
              <form onSubmit={(e) => { handleApply(e); setIsModalOpen(false); }} className="space-y-4">
                  {status && (
                    <div className={`p-3 rounded-xl border text-[11px] font-bold text-center ${status.includes('error') ? 'bg-red-500/10 border-red-500/20 text-red-400' : 'bg-green-500/10 border-green-500/20 text-green-400'}`}>
                      {status}
                    </div>
